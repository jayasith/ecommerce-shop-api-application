...
const {city,email,state,streetaddress} = props.delivery
    ...
    const codeHandle = async(e)=>{
        ...
        try{
            const mobileRespons = await fetch(
                `http//localhost:9090/rest/mobile`,
                {
                    method:"POST",
                    headers:{
            ...
            if(mobileRespons.ok){
                ...
                const orderResponse = await fetch(
                    `http//localhost:9090/rest/order`,
                    {
                        method:"POST",
                        headers: {
                            Accept: "application/json",
    ...
    return (
        <div>
            ...
            {(send) ?(<div className="cdForm">              
                    ...
                    <input type="phone" onChange={(e)=>setMobileno(e.target.value)} required  className="numberInput" />
                    <GoDeviceMobile className="mobileIcon"/>  
                    <button type="submit" className="sendBt" onClick={()=>setSend(false)}>send</button>            
                    ...
                    (<div className="cdForm" >
                        ...
                        <br/>              
                        <input type="text" required placeholder="4-digit" className="codeInput" onChange={(e)=>setCode(e.target.value)} />
                        <MdLockOutline className="lockIcon2" />
                        <br/>
                        <button className="backBt" onClick={()=>setSend(true)}>back</button>
                        <button onClick={(e)=>codeHandle(e)} className="submitBt">submit</button> 
