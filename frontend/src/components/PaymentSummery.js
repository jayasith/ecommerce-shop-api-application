	...
	return (
		<div>
			<form className="paymentform">
				...
				{context.carts.map((id) => {
					return (
						<div>
							...
							<label className="peoductName">
								{id.product.name} X {id.quantity}
							</label>
							<label className="subPrice">{id.product.itemprice}</label>
							<div className="cal">
								{(item += id.quantity)}
								{(subtotal += id.product.itemprice * id.quantity)}
							</div>{" "}
						</div>
					);
				})}
				<hr />
				<div className="fDiv">
					<label className="sub">Subtotal(LKR)</label>
				...
				<hr />
				<br />
				<label className="total">TOTAL(LKR)</label>
